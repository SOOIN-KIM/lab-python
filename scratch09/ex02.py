...
with open('../scratch08/mpg.csv', mode='r', encoding='UTF-8') as f:
    reader = csv.reader(f)
    reader.__next__() #한 줄 읽고 건너뜀.
    # 첫번째 줄은 컬럼 이름들이기 때문에.
    df = [line for line in reader]
print(df[0:5]) # 리스트 df에서 인덱스 0~4까지 행을 출력
# 리스트 df에서 0번째 행의 0,1,2 번째 컬럼 아이템만 출력
...
with open(file_path, mode='r', encoding='UTF-8') as f:
    ...
    # 보통 csv파일에 컬럼 이름이 포함된 경우 사용
    reader = csv.DictReader(f)
    # DictReader 객체의 read 기능을 사용하면,
    # 각 행은 '컬럼이름:값'(key:value)의 쌍으로 이루어진 dict가 됨.
