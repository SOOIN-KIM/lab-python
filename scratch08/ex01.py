...
def move(x,direction,step= -0.1):
    ''' x 좌표를 새로운 x로 이동.
    direction : 접선의 기우기.
    step > 0 인 경우는 접선과 같은 방향으로 이동 -> 최소값 찾기
    step < 0 인 경우는 접선과 반대 방향으로 이동 -> 최대값 찾기'''
...
if __name__ == '__main__':
    ...
    estimates_2 = [difference_quotient(f,x,h=0.1)
                   for x in xs]
    plt.scatter(xs, actuals, label = 'acutal',marker='x')
