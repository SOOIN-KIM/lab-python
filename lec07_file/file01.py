...
# 절대 경로(obsolute path):
    ...
    # C:\dev\lab-python\venv2\lec07_file <- 절대경로 방법(windows)
    # /Users/user/Documnets (MacOS 또는 Linux)
# 상대 경로(relative path):
...
print(os.path.isdir('')) #<- 현재 디렉토리는이냐
...
# 파일(폴더) 삭제 : os.remove(삭제할 파일/ 폴더 이름)
# 디렉토리 삭제 : os.rmdir(삭제할 폴더이름)
# try:
#     os.rmdir('test')
#     print('test 폴더가 삭제됨')
...
# os.mkdir('test1\\temp')
# test1 폴더가 없기 대문 그 하위 폴더를 생성할 수 없음
