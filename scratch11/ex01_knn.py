...
if __name__ == '__main__':
    # 1. 데이터 준비
    # csv 파일에 컬럼 이름들(헤더 정보)가 없기 때문에 컬럼 이름들을 정의
    ...
    # 3. 거리 계산을 위해서 각 특성(변수)들을 스케일링(표준화)
    ...
    # 5. 모델 평가
    conf_matrix = confusion_matrix(y_test,y_pred) #실제값, 예측값의 혼동행렬
    print(conf_matrix)
    ...
    # 5. 모델 향상( 성능 개선) - k값을 변화시킬 때 에러가 줄어드는 지.
    errors = []
    for i in range(1,31):
