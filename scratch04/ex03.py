...
def make_matrix(nrows,ncols, fn):
    '''
    함수 fn의 리턴 값들로 이루어진 nrows x ncols 행렬을 생성
...
if __name__ == '__main__':
    ...
    identity_matrix = make_matrix(3,3, identity)
...
def transpose(matrix):
    nrows = len(matrix) # 원본 행렬의 행의갯수
    ncols = len(matrix[0]) # 원본 행렬의 열의 갯수
    t = [] # 전치행렬
