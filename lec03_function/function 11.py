...
result = calc(1,2,lambda x,y : x+y) #plus라고 하는 함수를 만들어줄 필요가 없다.
print(result)
...
def my_filter(values, func):
    ...
    이루어진 새로운 리스트를 생성해서 리턴

    :param values: 리스트
    :param func: True or Flase를 리턴하는 함수
    :return: 필터링된 새로운 리스트
    '''

    result =[] # 빈 리스트를 만든다(새로운 리스트를 생성이란 말)
    for item in values: # 파라미터에 전달된 리스트의 모든 원소들에 대해서 반복
        if func(item):  # 필터링 조건 함수 func의 리턴 값을 검사
            result.append(item) # 조건이 true가 되는 조건들만 result 리스트에 append 해준다
    return result
...
def my_filter2(values,function):
    return [x for x in values if function(x)]
...
def my_mapper(values,func):
    ...
    리스트의 아이템들을 key, 함수의 리턴 값을 value로 하는
    dict를 리턴
